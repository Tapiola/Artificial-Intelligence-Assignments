...
def getNeighbours(features, x, y):
    """
    gets neighbours for specific coordinates
    """
    listy = []
    if not x == 0:
        if features[(x-1,y)] == features[(x,y)]:
            listy.append((x-1,y))
    if not x == 27:
        if features[(x+1,y)] == features[(x,y)]:
            listy.append((x+1,y))
    if not y == 0:
        if features[(x,y-1)] == features[(x,y)]:
            listy.append((x,y-1))
    if not y == 27:
        if features[(x,y+1)] == features[(x,y)]:
            listy.append((x,y+1))
...
def connected (features, visited, setty, x, y):
    """
    gets one connected component recursively
    """
    if visited[(x,y)] == False:
        setty.add((x,y))
        visited[(x,y)] = True
        for i,j in getNeighbours(features, x, y):
            connected(features, visited, setty, i, j)
...
def enhancedFeatureExtractorDigit(datum):
    ...
    ## 
        new added feature is the loop quantity
    ##
    """
    #initialisations
    ...
    #finds connected components, counts their quantity
    #adds it to the features
    for x,y in temp:
        if visited[(x,y)] == False:
            setty = set()
            connected(temp,visited,setty,x,y)
            connectedComponents.append(setty)

    loopCount = len(connectedComponents)-2
...
def enhancedPacmanFeatures(state, action):
    ...
    """
    features = util.Counter()
    pacman_curr = state.getPacmanPosition()
    pacman_new = state.generatePacmanSuccessor(action).getPacmanPosition()
    ghosts = state.getGhostPositions()
    food = state.getFood()
    capsules = state.getCapsules()
    # adds two features, one is more the further the ghost is away
    # the other is less the closer the ghost is
    for ghost in ghosts:
        dist_curr = util.manhattanDistance(ghost,pacman_curr)
        dist_new = util.manhattanDistance(ghost,pacman_new)
        features['away_from_ghost'] = max(features['away_from_ghost'] , 0.8  * (dist_new - dist_curr) / dist_curr)
        features['suicide'] = max(features['away_from_ghost'] , 0.09 * (dist_curr - dist_new) / dist_curr)
    # feature for food distance, is better the closer the food is
    for x,y in food.asList():
        if food[x][y]:
            dist_curr = util.manhattanDistance((x,y),pacman_curr)
            dist_new = util.manhattanDistance((x,y),pacman_new)
            features['food_distance'] = max(features['food_distance'], 1.2 *  (dist_curr - dist_new) / dist_curr)
    # feature for capsule distance, is better the closer the capsule is, has bigger priority than food
    for x,y in capsules:
        dist_curr = util.manhattanDistance((x,y),pacman_curr)
        dist_new = util.manhattanDistance((x,y),pacman_new)
        features['cap_distance'] = max(features['cap_distance'], 2.4 *  (dist_curr - dist_new) / dist_curr)                 
    # for stop agent
    if action == 'Stop':
        features['stop'] = 0.19
    else: features['stop'] = 0.0
    return features
