...
class ExactInference(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance == None:
        	# putting the ghost in the jail
        	pj = self.getJailPosition()
        	for p in self.legalPositions:
        		allPossible[p] = 0
        	allPossible[pj] = 1
        else:
        	# calculating beliefs according to the evidence
        	for p in self.legalPositions:
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        """
        # calculates beliefs according to timestep
        allPossible = util.Counter()
    ...
    def getBeliefDistribution(self):
    	# returns the belief distribution
        return self.beliefs
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        m = self.numParticles/n
        r = self.numParticles % n
    ...
    def elapseTime(self, gameState):
        ...
        """
        newParticles = []
        # resamples particles according to timestep
    ...
    def getBeliefDistribution(self):
        ...
        weights = util.Counter();
        for i in range(0,self.numParticles):
        	weights[self.particles[i]] = weights[self.particles[i]] + 1
        weights.normalize()
        return weights
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        weight with each position) is incorrect and may produce errors.
        """
        # initialises particles uniformly
        self.particles = []
        newPositions = []
        # gets position permutations
        for a in itertools.product(self.legalPositions, repeat=self.numGhosts):
        	newPositions.append(a)
        random.shuffle(newPositions)
        n = len(newPositions)
        m = self.numParticles/n
        r = self.numParticles % n
        # gets particles from positions
        for p in newPositions:
        	for i in range(0,m):
        		self.particles.append(p)
        for j in range(0,r):
        	self.particles.append(newPositions[j])
    ...
    def observeState(self, gameState):
        ...
        # reweighting for whole particles, going through and multiplying each ghost's p
        for i in range(0,self.numParticles):
        	prod = 1
        	for j in range(self.numGhosts):
        		trueDistance = util.manhattanDistance(self.particles[i][j], pacmanPosition)
        		p = busters.getObservationDistribution(noisyDistances[j])[trueDistance]
        		if noisyDistances[j] == None:
        			p = 1
        		prod = prod*p
        	weights[self.particles[i]] += prod
        weights .normalize()
        # the case where beliefs are rewighted to zero 
        if weights.totalCount() == 0:
        	self.initializeParticles()
    ...
    def elapseTime(self, gameState):
        ...
        for oldParticle in self.particles:
            ...
            # resamples particles according to the given BN
            prevGhostPositions = list(oldParticle)
            newPosDist = []
            for j in range(0,self.numGhosts):
                newPosDist = getPositionDistributionForGhost(setGhostPositions(gameState, prevGhostPositions), j, self.ghostAgents[j])
                newParticle[j] = util.sample(newPosDist)
            newParticles.append(tuple(newParticle))
        self.particles = newParticles

    def getBeliefDistribution(self):
    	""" 
			Returns beliefs according to the particles. Essentially, converts
			particles to beliefs according to their distribution.
    	"""
        weights = util.Counter()
        for i in range(self.numParticles):
        	weights[self.particles[i]] = weights[self.particles[i]] + 1
